...
void LevelTwoState::Initialise()
{
	C_SysContext::Get<Player>()->ResolveInCurrentLevel();
	...
	tileMap.loadFromFile("backgrounds/bg_ship.png");
	xmlPath = "backgrounds/bg_ship.xml";
	...
	SpawnedEnemies = map->SpawnObjectsFromVector(enemies, 15, 2);
	...
	totalEnemies = map->SpawnObjectsFromVector(enemies, 15, 2).size();
	player->SetTotalEnemies(totalEnemies);
}
